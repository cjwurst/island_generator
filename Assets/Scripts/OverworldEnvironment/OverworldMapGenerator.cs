...
public class OverworldMapGenerator : ScriptableObject
{
    ...
    public OverworldMap GenerateMap()
    {
        return null;
        /*var landPixels = MakeLandPixels();
        var faultPixels = MakeFaultPixels(landPixels, faultLSystem);*/
    }

    Dictionary<Vector2Int, bool> MakeLandPixels()
    {
        var landPixels = MakeFirstPixels();
        ...
        void Iterate(float density)
        {
            ...
            for (var i = 0; i < SIZE; i++)
                for (var j = 0; j < SIZE; j++)
                    if (landPixels[new Vector2Int(i, j)])
            ...
            var nearestSeeds = TextureHelper.GetNearestNeighbors(new Vector2Int(SIZE, SIZE), seeds, (s) => s.position, out var _);
        ...
        void ClearCell(Seed seed, Dictionary<Seed, HashSet<Vector2Int>> seedMap)
        {
            ...
            foreach (var pixel in pixels)
                landPixels[pixel] = false;
        }

        Dictionary<Vector2Int, bool> MakeFirstPixels()
        {
            var pixels = new Dictionary<Vector2Int, bool>();
            for (var i = 0; i < SIZE; i++)
                for (var j = 0; j < SIZE; j++)
                    pixels.Add(new Vector2Int(i, j), seedTexture.GetPixel(i, j) == Color.black);
            for (var i = 0; i < deadPixelCount; i++)
                pixels[new Vector2Int(Random.Range(0, SIZE - 1), Random.Range(0, SIZE - 1))] = false;
            return pixels;
        }
    }
    ...
    void MakeFaultTexture(Dictionary<Vector2Int, bool> landPixels, LSystem faultLSystem)
    {
        /*var faultPixels = new Dictionary<Vector2Int, bool>();
        for (var tryCount = 0; tryCount < maxTries && !ValidateFault(); tryCount++)
        {
            for (var i = 0; i < SIZE; i++)
                for (var j = 0; j < SIZE; j++)
                    faultPixels[new Vector2Int(i, j)] = false;

            var stringToDraw = faultLSystem.axiom;
            var origin = new Vector2Int(Random.Range(0, SIZE), Random.Range(0, SIZE));
    ...
    class Seed
    {
        ...
        {
            var x = Random.Range(0, boundSize + 1);
            var y = Random.Range(0, boundSize + 1);
